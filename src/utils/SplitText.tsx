    ...
    >
      {chars.map((char, index) => (
        <SplitFlapChar
          ...
          settledColor={index === 0 || index === 1 ? "#ffa212" : undefined}
        />
      ))}
    </div>
  )
...
function SplitFlapChar({ char, index, animationKey, skipEntrance, speed, playClick, settledColor }: SplitFlapCharProps) {
  ...
  const bgColor = isSettled ? "hsl(0, 0%, 0%)" : "rgba(255, 162, 18, 0.2)"
  const textColor = isSettled ? (settledColor || "#ffffff") : "#ffa212"
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-0 top-0 bottom-1/2 flex items-end justify-center overflow-hidden w-full">
        <span
          className="block translate-y-[0.52em] transition-colors duration-150"
          style={{ color: textColor, lineHeight: 1, whiteSpace: "nowrap", textAlign: "center", width: "100%" }}
        >
          {currentChar}
        </span>
      </div>

      <div className="absolute inset-x-0 top-1/2 bottom-0 flex items-start justify-center overflow-hidden w-full">
        <span
          className="block -translate-y-[0.52em] transition-colors duration-150"
          style={{ color: textColor, lineHeight: 1, whiteSpace: "nowrap", textAlign: "center", width: "100%" }}
        >
      ...
      <motion.div
        ...
        className="absolute inset-x-0 top-0 bottom-1/2 origin-bottom overflow-hidden w-full"
        style={{
          width: "100%",
          backgroundColor: bgColor,
          transformStyle: "preserve-3d",
